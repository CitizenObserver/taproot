...
import typer
from rich import print  # noqa: TID252
...
app = typer.Typer(help="Taproot: quick EC2 Instance Connect via TUI.", add_completion=False)


@app.command()
def connect(
    profile: Optional[str] = typer.Option(None, "-p", "--profile", help="AWS CLI profile name."),
    instance_id: Optional[str] = typer.Option(None, "--instance-id", help="Connect directly, skip TUI."),
    ...
    if instance_id is None:
        ...
        all_ids = {inst["id"] for inst in collect_instances(session)}
        if instance_id not in all_ids:
            typer.echo(f"Instance {instance_id} not found under profile {profile}", err=True)
            raise typer.Exit(code=2)

    cache_save(profile, instance_id)
    _launch_ssh(profile, instance_id, user)


def _launch_ssh(profile: str, instance_id: str, user: str) -> None:
    cmd = [
        "aws",
